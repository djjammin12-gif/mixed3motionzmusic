...
<body>
  ...
  <main id="top" data-sb-object-id="${objectId}">
    ...
    <section class="merch section-dark" id="merch" aria-labelledby="merch-title"><div class="section-heading"><h2 id="merch-title" ${field("merch.title")}>${esc(site.merch.title)}</h2><p ${field("merch.subheadline")}>${esc(site.merch.subheadline)}</p></div><div class="category-row">${map(site.merch.categories, (cat, i) => `<button type="button" ${field(`merch.categories.${i}`)}>${esc(cat)}</button>`)}</div><div class="merch-grid">${map(site.merch.items, (item, i) => `<article class="merch-card"><div class="product-shot"><img src="/assets/product-flatlay.svg" alt="${attr(item.name)} product placeholder" loading="lazy" /></div><span class="artifact" ${field(`merch.items.${i}.artifact`)}>${esc(item.artifact)}</span><h3 ${field(`merch.items.${i}.name`)}>${esc(item.name)}</h3><dl><div><dt>Front</dt><dd ${field(`merch.items.${i}.front`)}>${esc(item.front)}</dd></div><div><dt>Back</dt><dd ${field(`merch.items.${i}.back`)}>${esc(item.back)}</dd></div></dl><p ${field(`merch.items.${i}.description`)}>${esc(item.description)}</p><div class="buy-row"><select aria-label="${attr(item.name)} size"><option>Size</option><option>S</option><option>M</option><option>L</option><option>XL</option></select><span ${field(`merch.items.${i}.price`)}>${esc(item.price)}</span></div><button type="button">Join waitlist</button></article>`)}</div><p class="scarcity">${esc(site.merch.scarcity.join(" / "))}</p></section>
